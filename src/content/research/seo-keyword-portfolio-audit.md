...
2.  [What a healthy portfolio looks like](#design)
3.  [What the 90/90 pattern signals](#pattern)
4.  [Conductor mechanics that shape every number](#conductor)
5.  [The audit and rebuild sequence](#audit)
6.  [Why this matters beyond SEO](#aeo)
7.  [Next steps and open questions](#next)
8.  [Sources](#sources)
...
Three findings carry the weight. First, the enterprise literature is consistent on portfolio composition: seoClarity's framework allocates roughly 30% to current winners, 30% to page-two optimization targets, 30% to deliberate competitive gaps, and 10% to experiments, while BrightEdge's rule of thumb caps branded terms at 1–2% of tracked quota.<sup>[3](#s3),[4](#s4)</sup> Groups pinned near 0% or 100% ranked at scale fit neither shape. Second, Conductor's own docs contain the hygiene rules a healthy account runs on: deactivate keywords with zero search volume or no ranking in 6–12 months; segment brand from non-brand with smart groups; map preferred URLs; and note that their Content Opportunity Score deliberately weights toward positions 11–20, the striking-distance segment where effort converts to movement.<sup>[1](#s1)</sup> Third, two structural mechanics in Conductor shape every reported number: a keyword tracked across multiple locations, devices, or engines counts as multiple tracked keywords (so group sizes may be partly context multiplication), and the market share metric uses the tracked list as its denominator, so a brand-heavy list mechanically manufactures dominant "market share."<sup>[1](#s1),[2](#s2)</sup>
...
Four mechanics from Conductor's documentation matter for reading any report built on this account, all verified in their knowledge base.<sup>[1](#s1),[2](#s2)</sup>
...
Assembled from Conductor's rules plus independent frameworks (HawkSEM's audit steps, Clearscope on striking distance, seoClarity's refresh loop), in order:<sup>[1](#s1),[3](#s3),[5](#s5)</sup>
